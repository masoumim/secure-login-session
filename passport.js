...
passport.use(new LocalStrategy(async function (username, password, done) {
    try {
        ...
        return done(null, { id: user.rows[0].id, name: user.rows[0].name, password: user.rows[0].password });
    } catch (err) {
...
passport.deserializeUser(async (id, done) => {
    try {
        ...
        done(null, { id: user.rows[0].id, name: user.rows[0].name, password: user.rows[0].password });
    } catch (err) {
        done(err);
    }
});